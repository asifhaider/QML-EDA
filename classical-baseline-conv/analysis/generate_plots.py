...
prediction maps, and the 5-fold CV per-city bar chart, for the 3x3 conv model.

Port of classical-baseline-mlp/analysis/generate_plots.py for this model —
same design rationale (see that file's docstring for the full reasoning
behind each chart form and the palette). The one functional difference:
`predict_city` (dense, overlap-averaged) is used instead of
`predict_city_center`, matching this model's native dense (H,W) output —
consistent with train/trainer.py and train/eval_full.py's own choice.
"""
import os, sys, json
import numpy as np
import matplotlib
...
# -- fixed palette, identical to the MLP directory's for cross-report consistency --
BLUE = "#2563eb"
AMBER = "#d97706"
MAGENTA = "#db2777"
NEARWHITE = "#f3f4f6"
...
def get_pooled_pilot_predictions():
    ...
    cities via predict_city (dense, overlap-averaged), return pooled (P, Y)
    plus tau* and per-city P/Y/mask for the qualitative panel."""
    ...
    params = np.load(os.path.join(RUNS, "conv_pca_pilot_bestcheap.npy"))
    forward = lambda p, Xb, Sb: conv_model.forward(p, Xb, Sb)

    per_city = {}
    allp, ally = [], []
    for c in val_cities:
        P = predict_city(forward, params, Xva[c], Sva[c])
        Y = fold.labels[c]
        m = fold.valid[c]
...
def plot_confusion_matrix(P, Y, tau):
    ...
    fig.suptitle(f"Conv3x3 — pooled confusion matrix (dev split, τ*={tau:.3f})", fontsize=11, x=0.46)
    vmax = mat.max()
    for i in range(2):
        for j in range(2):
            val = mat[i, j]
...
def plot_pr_roc(P, Y):
    ...
    fig.suptitle("Conv3x3 — pooled dev-split curves", fontsize=12, y=1.02)
    fig.tight_layout()
...
def plot_qualitative(per_city, tau, val_cities):
    ...
    fig.suptitle("Conv3x3 — qualitative predictions, dev-split best vs. worst city", fontsize=12)
...
def plot_cv_bar_chart():
    ...
    ax.legend(loc="lower right", frameon=False, fontsize=9)
    ax.set_title("Conv3x3 — per-city AP, 5-fold city-grouped CV (all 14 labelled cities)", fontsize=11)
    fig.tight_layout()
